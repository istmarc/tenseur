...
template <class A, class B>
   requires(::ten::isStaticTensor<A>::value && ::ten::isStaticTensor<B>::value)
testing::AssertionResult same_shape(const A &a, const B &b) {
   if (A::rank() != B::rank())
      return testing::AssertionFailure() << "Different rank " << A::rank()
                                         << " and " << B::rank() << std::endl;

   if (A::staticSize() != B::staticSize())
      return testing::AssertionFailure()
             << "Different sizes " << A::staticSize() << " and "
             << B::staticSize() << std::endl;
...
// Compare two tensors
template <class A, class B>
   requires(::ten::isDynamicTensor<A>::value &&
            ::ten::isDynamicTensor<B>::value)
